...
MODEL_DATA_DIR = osp.join(BASE_DIR, "model_data")
if not osp.exists(MODEL_DATA_DIR):
    os.makedirs(MODEL_DATA_DIR)

from face_detection.nets.facenet import Facenet
from face_detection.nets_retinaface.retinaface import RetinaFace
from face_detection.utils.config import cfg_mnet, cfg_re50
from face_detection.utils.anchors import Anchors
from face_detection.utils.utils import (Alignment_1, compare_faces, letterbox_image,
                         preprocess_input)
...
def cv2ImgAddText(img, label, left, top, textColor=(255, 255, 255)):
    ...
    #   设置字体
    #---------------#
    font_path = osp.join(MODEL_DATA_DIR, 'simhei.ttf')
...
class Retinaface(object):
    _defaults = {
        ...
        #   retinaface训练完的权值路径
        #----------------------------------------------------------------------#
        "retinaface_model_path" : osp.join(MODEL_DATA_DIR, 'Retinaface_mobilenet0.25.pth'),
        #----------------------------------------------------------------------#
        ...
        #   facenet训练完的权值路径
        #----------------------------------------------------------------------#
        "facenet_model_path"    : osp.join(MODEL_DATA_DIR, 'facenet_mobilenet.pth'),
        #----------------------------------------------------------------------#
    ...
    #   初始化Retinaface
    #---------------------------------------------------#
    def __init__(self, encoding=False, **kwargs):
        ...
        try:
            face_encoding_path = osp.join(MODEL_DATA_DIR, "{backbone}_face_encoding.npy".format(backbone=self.facenet_backbone))
            names_path = osp.join(MODEL_DATA_DIR, "{backbone}_names.npy".format(backbone=self.facenet_backbone))
            
            self.known_face_encodings = np.load(face_encoding_path)
            self.known_face_names = np.load(names_path)
        except:
            if not encoding:
    ...
    def encode_face_dataset(self, image_paths, names):
        ...
        # 保存编码文件
        face_encoding_path = os.path.join(model_data_path, "{backbone}_face_encoding.npy".format(backbone=self.facenet_backbone))
        names_path = os.path.join(model_data_path, "{backbone}_names.npy".format(backbone=self.facenet_backbone))
        
        np.save(face_encoding_path, face_encodings)
